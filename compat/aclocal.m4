...
[
	...
	AC_MSG_CHECKING([if has tcf_police_burst])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <net/tc_act/tc_police.h>
	],[
		tcf_police_burst(NULL);
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_TCF_POLICE_BURST, 1,
	...
	*)
	...
	AC_MSG_CHECKING([if netdevice.h has netif_napi_add_tx])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		#include <linux/netdevice.h>
	],[
		netif_napi_add_tx(NULL, NULL, NULL);

		return 0;
	],[
		AC_MSG_RESULT(yes)
		MLNX_AC_DEFINE(HAVE_NETIF_NAPI_ADD_TX, 1,
	...
	AC_MSG_CHECKING([if svc_fill_write_vector use 4 params])
	MLNX_BG_LB_LINUX_TRY_COMPILE([
		...
		MLNX_AC_DEFINE(HAVE_SVC_FILL_WRITE_VECTOR, 1,
			[svc_fill_write_vector use 4 params])
	],[
		AC_MSG_CHECKING([if svc_fill_write_vector use 3 params])
		MLNX_BG_LB_LINUX_TRY_COMPILE([
			#include <linux/sunrpc/svc.h>
		],[
